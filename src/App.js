...
// ── Palette ──────────────────────────────────────────────────────────────────
const C = {
  blue:       '#1A5CDB',   // primary blue
  blueDark:   '#1347B0',   // hover blue
  blueLight:  '#EBF1FF',   // card / subtle bg
  blueMid:    '#BFCFEE',   // border
  yellow:     '#F5C800',   // primary yellow
  yellowDark: '#D4A900',   // border / hover
  yellowLight:'#FFF8CC',   // yellow tint
  ink:        '#0E1B2E',   // main heading text
  body:       '#2C3E57',   // body text
  muted:      '#5A718A',   // secondary text
  bg:         '#F5F3EE',   // warm off-white page bg
  bgSection:  '#EDE9E1',   // slightly darker warm section
  white:      '#FFFFFF',
};
...
// ── VIDEO CONFIG ─────────────────────────────────────────────────────────────
// Replace these with your actual video file URLs.
// Works with: a direct MP4 link from Dropbox, Google Drive, your own web server, etc.
// Dropbox tip:  change ?dl=0 to ?raw=1 at the end of your share link
// Google Drive: change /file/d/FILE_ID/view  to  /uc?export=download&id=FILE_ID
const VIDEOS = {
...
// ─── YOUTUBE MODAL ────────────────────────────────────────────────────────────
// ─── VIDEO MODAL (supports Vimeo, Google Drive, and direct MP4s) ─────────────
const VideoModal = ({ src, title, onClose }) => {
  ...
  const getEmbedSrc = () => {
    if (isVimeo) {
      // Accept either full URL (vimeo.com/123456789) or just the ID (123456789)
    ...
    <nav style={{ background: C.white, borderBottom: `3px solid ${C.yellow}`, position: 'sticky', top: 0, zIndex: 40, fontFamily: FONT }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '62px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', flexShrink: 0 }} onClick={() => setCurrentPage('home')}>
          {/* ── LOGO IMAGE ── Replace the src below with your logo file path or URL */}
          <img
            src="/logo.png"
            alt="Group Project logo"
            style={{ height: '40px', width: 'auto', display: 'block', flexShrink: 0 }}
            onError={e => {
              // If image fails to load, fall back to the GP text badge
              e.currentTarget.style.display = 'none';
              e.currentTarget.nextSibling.style.display = 'flex';
            }}
          />
          {/* Fallback badge — hidden when image loads successfully */}
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="card-lift"
      ...
      <div onClick={() => onPlay(clip)}
        style={{ height: '220px', background: `linear-gradient(135deg, ${C.blue} 0%, ${C.blueDark} 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', position: 'relative', overflow: 'hidden' }}>
        <Film size={80} color="rgba(255,255,255,0.06)" style={{ position: 'absolute' }} />
        <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
          <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: C.yellow, border: `3px solid ${C.yellowDark}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 0 6px rgba(245,200,0,0.2)`, transition: 'transform 0.2s', transform: hovered ? 'scale(1.1)' : 'scale(1)' }}>
            <Play size={22} fill={C.ink} color={C.ink} style={{ marginLeft: '3px' }} />
          </div>
          <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '12px', fontFamily: FONT, fontWeight: 600 }}>{clip.duration}</span>
        </div>
        <div style={{ position: 'absolute', top: '14px', left: '14px', background: C.yellow, border: `2px solid ${C.yellowDark}`, borderRadius: '6px', padding: '3px 10px' }}>
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: FONT }}>
      ...
      <section style={{ background: C.bg, borderBottom: `3px solid ${C.yellow}` }}>
        <div className="hero-grid">
          ...
          {/* Video thumbnail */}
          <div onClick={() => setModal({ src: VIDEOS.main, title: 'Episode 12 — Final Draft' })}
            style={{ aspectRatio: '16/9', background: `linear-gradient(135deg, ${C.blue} 0%, ${C.blueDark} 100%)`, borderRadius: '16px', overflow: 'hidden', cursor: 'pointer', position: 'relative', boxShadow: `8px 8px 0 ${C.ink}`, border: `3px solid ${C.ink}`, transition: 'all 0.2s' }}
            onMouseEnter={e => { e.currentTarget.style.transform = 'translate(-3px,-3px)'; e.currentTarget.style.boxShadow = `11px 11px 0 ${C.ink}`; }}
            onMouseLeave={e => { e.currentTarget.style.transform = 'translate(0,0)'; e.currentTarget.style.boxShadow = `8px 8px 0 ${C.ink}`; }}>
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '14px' }}>
              <div style={{ width: '68px', height: '68px', borderRadius: '50%', background: C.yellow, border: `3px solid ${C.yellowDark}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 0 8px rgba(245,200,0,0.2)' }}>
              ...
              <div style={{ textAlign: 'center', padding: '0 16px' }}>
                ...
                <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '13px', marginTop: '4px', fontFamily: FONT }}>Season Finale</p>
              </div>
            </div>
            <div style={{ position: 'absolute', top: '12px', left: '12px', background: C.yellow, color: C.ink, fontSize: '10px', fontWeight: 900, padding: '5px 12px', borderRadius: '6px', letterSpacing: '0.08em', fontFamily: FONT, border: `2px solid ${C.yellowDark}` }}>SEASON FINALE</div>
          </div>
        </div>
      </section>

      {/* Stats — solid yellow band */}
      ...
      <section className="section-pad" style={{ background: C.bg }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          ...
          <div className="announcements-grid">
            ...
            <AnnouncementCard title="Season 2 Confirmed" date="Dec 5, 2024" category="NEWS" description="Due to popular demand, Group Project will return for a second season next fall. More chaos coming soon." accent="#D4A900" />
          </div>
        </div>
      </section>
    </div>
